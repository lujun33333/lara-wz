...
Require-Text 'lara/kexploit/WZHUDBridge.mm' '\[drawRoot addSubview:g_panel\]' 'Core 菜单视觉层仍未放入最低绘制窗口'
...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'g_active\.load\(\) && g_backgroundWindowPolicyReady\.load\(\)' '后台窗口策略失败后仍会伪报 HUD 已运行'
...
Reject-Text 'lara/kexploit/WZHUDBridge.mm' 'SBSAccessibilityWindowHostingController|registerWindowWithContextID:' '错误混入 AX SpringBoard 窗口托管链'
...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'g_displayLink\.paused = YES' '进入后台仍持续更新而非保留最后一帧'
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'DISPATCH_SOURCE_TYPE_TIMER' '后台缺少 Core 式独立渲染定时器'
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'render_frame_main\(CACurrentMediaTime\(\)\)' '后台定时器没有消费最新绘制快照'
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'dispatch_source_cancel\(g_backgroundRenderTimer\)' 'HUD 销毁时未停止后台渲染定时器'
