...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'CALayer \*layers\[\] = \{[\s\S]*g_renderSurfaceLayer, g_canvas\.layer, g_floatButton\.layer,[\s\S]*\};' '后台图层 mask 未限制为 Core 的 qp420、QXA110 主层和浮球层'
...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'synchronize_orientation_main\(\);\s*BOOL layerMaskReady = configure_background_layer_masks_main\(0x12\);\s*BOOL firstPublish = g_window\.hidden;\s*BOOL contentReady = update_fallback_snapshot_main\(YES\);[\s\S]*?g_window\.hidden = NO;' 'Core 绘制窗口首次可见前未完成菜单像素、布局和图层掩码'
...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'if \(!configure_metal_renderer_main\(\)\) \{\s*[\s\S]*?g_window\.hidden = YES;\s*g_controlWindow\.hidden = YES;\s*g_floatWindow\.hidden = YES;' 'Metal 初始化失败后未关闭已发布的 Core 系统窗口'
...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'g_active\.load\(\) && g_backgroundLayerMaskReady\.load\(\) &&\s*g_firstCommitContentReady\.load\(\)' '首次菜单内容或图层掩码失败后仍会伪报 HUD 已运行'
...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'commitValid = g_requested\.load\(\) && g_sceneActive\.load\(\)[\s\S]*g_contextsStable\.load\(\)' '并发关闭或退后台时缺少托管提交前复核'
