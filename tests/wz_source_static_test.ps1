...
Require-Text 'scripts/WZHUDHostHelper.m' '__attribute__\(\(constructor\)\)[\s\S]*_NSGetArgc\(\)[\s\S]*--wzhud-host[\s\S]*WZHUDHostProcessMain[\s\S]*_exit\(exitCode\)' 'HUD host 模式未在 Swift @main 前分流并终止子进程入口'
Require-Text 'scripts/WZHUDHostHelper.m' 'registerWindowWithContextID:atLevel:[\s\S]*v@:Id[\s\S]*SBSAccessibilityWindowHostingController' '主 executable HUD 模式未按 TrollSpeed 精确注册 context'
Require-Text 'scripts/WZHUDHostHelper.m' 'termSource[\s\S]*interruptSource[\s\S]*parentSource[\s\S]*WZWriteReadyFile[\s\S]*host-mode=ready[\s\S]*CFRunLoopRun' 'HUD host 在发布 ready 前未完整安装信号、父进程与运行循环生命周期'
Reject-Text 'scripts/WZHUDHostHelper.m' '^int main\(|RemoteCall|SBMainWorkspace|UIButton' 'HUD host 模式不应定义第二 main、注入 SpringBoard 或创建菜单控件'
...
Require-Text 'lara.xcodeproj/project.pbxproj' 'scripts/WZHUDHostHelper\.m in Sources' 'HUD host 入口未编入 Lara 主 target'
Require-Text 'scripts/build_ipa_wz.sh' 'WZHUDHostHelper\.o[\s\S]*--wzhud-host[\s\S]*SBSAccessibilityWindowHostingController' '构建脚本未验证主 executable 包含完整 HUD host 模式'
Require-Text 'scripts/build_ipa_wz.sh' '! -e "\$SRC_APP/WZHUDHostHelper"[\s\S]*! -e "\$STAGE/Payload/\$APP\.app/WZHUDHostHelper"' '打包脚本未拒绝独立 WZHUDHostHelper 裸文件'
Reject-Text 'scripts/build_ipa_wz.sh' 'HUD_HELPER=|clang[\s\S]{0,200}WZHUDHostHelper\.m|ldid -S[^\r\n]*HUD_HELPER|chmod 0755[^\r\n]*WZHUDHostHelper' '构建仍在生成、签名或修复独立 HUD helper'
...
Require-Text 'scripts/build_ipa_wz.sh' '--wzhud-host[\s\S]*SBSAccessibilityWindowHostingController[\s\S]*registerWindowWithContextID:atLevel:' '打包脚本未验证主 executable 的双模式 context 托管标记'
