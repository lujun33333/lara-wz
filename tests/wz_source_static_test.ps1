...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'BOOL shouldUseDrawable = appState == UIApplicationStateActive;[\s\S]*shouldUseDrawable &&[\s\S]*present_snapshot_metal_frame_main\(\)' '后台仍会请求 nextDrawable，未对齐 Core qx309 的 offscreen fallback 路径'
...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'synchronize_orientation_main\(\);\s*BOOL layerMaskReady = configure_background_layer_masks_main\(0x12\);\s*g_window\.hidden = NO;' 'Core 绘制窗口首次可见前未完成内容布局和图层掩码'
...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'BOOL contextsPreserved = wasStable &&\s*g_windowContextIDs[\s\S]*schedule_context_validation_main\(\);' '前台恢复仍用 drawable 结果否定稳定 context，而非按 ID 变化重验'
