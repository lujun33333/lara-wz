...
# mirrors. The SBS controller is only a legacy fallback on older systems.
...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'set_background_update_mask_main\(NSInteger mask\)[\s\S]{0,500}setDisableUpdateMask:' '缺少 Core 后台图层掩码实现'
...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'g_window = applicationScene[\s\S]{0,150}initWithWindowScene:applicationScene[\s\S]{0,200}g_menuWindow = applicationScene[\s\S]{0,150}initWithWindowScene:applicationScene' 'Lara 本地双窗口未恢复 scene-bound 可见性'
Require-Text 'lara/kexploit/WZHUDBridge.mm' '@implementation WZHUDDrawWindow[\s\S]{0,240}canBecomeKeyWindow \{ return NO; \}' '绘制窗口仍会抢占 Lara key window'
Require-Text 'lara/kexploit/WZHUDBridge.mm' '@implementation WZHUDMenuWindow[\s\S]{0,240}canBecomeKeyWindow \{ return NO; \}' '菜单窗口仍会抢占 Lara key window'
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'if \(!active\)[\s\S]{0,500}set_background_update_mask_main\(0x12\)' 'iOS 26 system-window 后台未启用 Core 保活掩码'
...
# Game launch must never be gated by a private HUD class or context host.
Require-Text 'lara/classes/laramgr.swift' 'func launchWZGame\(\)[\s\S]*setGameHUD\(true\)[\s\S]*launch is independent of HUD[\s\S]*openWZGameURL\(url, epoch: launchEpoch\)' '游戏 URL 仍未与 HUD 就绪状态解耦'
Reject-Text 'lara/classes/laramgr.swift' 'prepareWZLocalHUDAndOpen|attempt < 30|正在注册 AX 本地双窗口' '游戏启动仍保留 HUD 等待门禁'
Require-Text 'lara/classes/laramgr.swift' 'setGameHUD\(true\)[\s\S]{0,120}prepareWZSpringBoardHosting\(\)[\s\S]{0,700}openWZGameURL\(url, epoch: launchEpoch\)' 'SpringBoard 托管未与游戏 URL 并行启动'
Require-Text 'lara/classes/laramgr.swift' 'wzhud_register_springboard_hosts\(remoteProcess\)' 'Swift 未调用 AX SpringBoard 托管链'
Require-Text 'lara/classes/laramgr.swift' 'opening smoba URL[\s\S]*open callback opened=' '游戏 URL 启动缺少请求与回调诊断'
...
Require-Text 'lara/classes/laramgr.swift' 'private func hideGameHUD\([\s\S]{0,1200}wzhud_set_enabled\(false\)' 'HUD 停止未直接走 wzhud_set_enabled(false)'
...
Require-Text 'lara/lara.swift' 'wzGameHUDKeepsRemoteCallAlive \|\| wzhud_springboard_hosting_ready\(\)' 'HUD 活跃时仍会在退后台销毁 SpringBoard RemoteCall'
Require-Text 'lara/kexploit/TaskRop/RemoteCall.m' 'return version\.majorVersion >= 16;' 'iOS 17-26 仍错误路由到 legacy RemoteCall'
