...
Require-Text 'lara/kexploit/WZHUDBridge.mm' '_setAllWindowsKeepContextInBackground:' '未同步 Core 后台窗口上下文保活'
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'g_directInteractionEnabled = YES' '前台 UIKit 直接交互仍被禁用'
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'g_active\.load\(\) && g_keepContextsInBackground\.load\(\)' '后台窗口上下文失败后仍会伪报 HUD 已运行'
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'g_controlWindow\.rootViewController = nil' '控制窗口销毁时未解除控制器'
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'set_keep_contexts_in_background_main\(NO\)' 'HUD 关闭时未撤销后台窗口保活'
