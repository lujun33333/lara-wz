...
Require-Text 'lara/kexploit/WZHUDBridge.mm' 'g_window = \[\[WZHUDDrawWindow alloc\][\s\S]{0,100}initWithFrame:UIScreen\.mainScreen\.bounds\][\s\S]{0,120}g_menuWindow = \[\[WZHUDMenuWindow alloc\][\s\S]{0,100}initWithFrame:UIScreen\.mainScreen\.bounds\]' 'HUD 双窗口仍绑定应用 UIWindowScene，未对齐 AX 的 scene-less context'
Reject-Text 'lara/kexploit/WZHUDBridge.mm' '\[\[WZHUD(Draw|Menu)Window alloc\] initWithWindowScene:' 'HUD 窗口仍通过 initWithWindowScene 创建'
Require-Text 'lara/kexploit/WZHUDBridge.mm' '@implementation WZHUDDrawWindow[\s\S]{0,240}canBecomeKeyWindow \{ return YES; \}' '绘制窗口未对齐 AX 的 key-window 能力'
Require-Text 'lara/kexploit/WZHUDBridge.mm' '@implementation WZHUDMenuWindow[\s\S]{0,240}canBecomeKeyWindow \{ return YES; \}' '菜单窗口未对齐 AX 的 key-window 能力'
