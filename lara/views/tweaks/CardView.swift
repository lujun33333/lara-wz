...
struct CardView: View {
    ...
    private func restorepassjson(card: carditem) {
        ...
        if writeprefersbx(path: passjsonpath(for: card), data: data) {
            clearcache(for: card)
            currentcardnum = readcardnum(for: card) ?? ""
            status = "已还原 pass.json"
        } else {
            status = "还原 pass.json 失败"
        }
    }
