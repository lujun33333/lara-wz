...
struct JitView: View {
        ...
        NavigationStack {
            List {
                ...
                Section {
                    ...
                    Text("仅对带有 `get-task-allow` 权限的应用有效。")
                }
            }
    ...
    private func enablejit(bundleID: String) {
        ...
        guard mgr.dsready else {
	            globallogger.log("内核读写未就绪")
	            return
	        }

	        enablingbid = bundleID
	        globallogger.log("(jit) 正在为 \(bundleID) 启用…")

	        let runenable: () -> Void = {
				guard let sbProc = mgr.sbProc else {
					globallogger.log("(jit) 错误：sbProc 为 nil")
				...
				DispatchQueue.global(qos: .userInitiated).async {
					...
					DispatchQueue.main.async {
						if err == 0 {
							globallogger.log("(jit) 已为 \(bundleID) 启用")
						} else {
							globallogger.log("(jit) 为 \(bundleID) 启用时出错！")
						}
						enablingbid = nil
					}
				}
	        ...
	        if mgr.rcrunning {
	            ...
	            mgr.rcinit(process: "SpringBoard", migbypass: false) { success in
	                if success {
	                    ...
	                    globallogger.log("(jit) rcinit 失败")
	                    enablingbid = nil
	                }
	            }
        }
