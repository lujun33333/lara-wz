...
struct VarCleanView: View {
    ...
    var body: some View {
        List {
            ...
            if groups.isEmpty && !isrefreshing {
                Section("匹配项") {
                    Text("未发现来自 VarCleanRules.json 的黑名单残留。")
                        .foregroundColor(.secondary)
                }
            } else {
        ...
        .alert("删除所选项目？", isPresented: $showdeleteconfirm) {
            Button("取消", role: .cancel) {}
            Button("删除", role: .destructive) {
                Task { await deleteselected() }
            }
        } message: {
            Text("删除 \(selectedcount) 个匹配路径\(selectedcount == 1 ? "" : "")？")
        }
    }
    ...
    private func refresh() async {
        ...
        if groups.isEmpty {
            ...
            statusmsg = "找到 \(matchcount) 个匹配路径\(matchcount == 1 ? "" : "")。"
        }
    }
    ...
    private func deleteselected() async {
        ...
        if failures.isEmpty {
            statusmsg = "已删除 \(deletedcount) 个路径\(deletedcount == 1 ? "" : "")。"
        } else {
            statusmsg = "已删除 \(deletedcount) 个路径\(deletedcount == 1 ? "" : "")，失败 \(failures.count) 个。"
        }
