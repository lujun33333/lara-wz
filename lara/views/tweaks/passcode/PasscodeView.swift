    ...
    func restoreAll(basePath: String, logmsg: ((String) -> Void)? = nil) throws {
        ...
        guard let enumerator = fm.enumerator(atPath: basePath) else {
            throw NSError(domain: "PasscodeTheme", code: 3,
                          userInfo: [NSLocalizedDescriptionKey: "无法枚举缓存"])
        }
            ...
            Form {
                ...
                Section(header: Text("应用")) {
                    ...
                    if !statusMessage.isEmpty {
                        Text(statusMessage)
                            .foregroundColor(statusMessage.contains("失败") || statusMessage.contains("错误") ? .red : .green)
                            .font(.footnote)
                    }
                }
                
                Section(header: Text("危险区域")) {
                    Button("清除所有键", role: .destructive) {
                        selectedKeys.removeAll()
                    }
                    Button("恢复原始图标", role: .destructive) { restoreTheme() }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    processing = false
                    statusMessage = "已导入 \(extractedKeys.count) 个键"
                }
            } catch {
                DispatchQueue.main.async {
                    processing = false
        ...
        DispatchQueue.global(qos: .userInitiated).async {
                ...
                autoreleasepool {
                    ...
                    for path in matched {
                        do {
                            ...
                            successCount += 1
                            mgr.logmsg("已应用 \(keyId) -> \(path)")
                        } catch {
                            failCount += 1
                            errors.append("\(path): \(error.localizedDescription)")
                            mgr.logmsg("失败 \(path)：\(error.localizedDescription)")
                        }
                    }
                }
            }
            ...
            DispatchQueue.main.async {
                ...
                if failCount == 0 {
                    ...
                    passcodeThemeManager.message = "完成，但有错误"
    ...
    func resolveTelephonyBasePath() -> String? {
        for version in telephonyOptions {
            ...
            if sbxdirExists(path: path) {
                mgr.logmsg("TelephonyUI 缓存：\(path)")
                return path
            }
        }
