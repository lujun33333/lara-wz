...
struct DarkBoardView: View {
    ...
    private var helperCards: some View {
        VStack(spacing: 12) {
            if !mgr.sbxready {
                Text("请先初始化 SBX。该图标主题工具使用基于 SBX 的文件读写，并在注销重启修复后恢复备份。")
                    .font(.footnote)
            ...
            if manager.themes.isEmpty {
                Text("导入包含 `IconBundles/<bundle-id>.png` 图标的文件夹、`.theme` 或 `.zip`。")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                    .padding()
                    .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private func applyThemes() {
        guard mgr.sbxready else {
            alert = DarkBoardAlert(message: "SBX 尚未初始化。请先运行漏洞利用并初始化 SBX，然后再应用。")
            return
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.35) {
                    if errors.isEmpty {
                        alert = DarkBoardAlert(message: "图标已应用。请立即注销重启。重新打开 lara 后，请再次初始化 SBX，以便注销重启后的图标修复能够恢复原始的 bundle 文件。")
                    } else {
                        alert = DarkBoardAlert(message: "应用时出现部分错误：\n\n" + errors.joined(separator: "\n\n"))
                    }
                }
            } catch {
                ...
                Button {
                    ...
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            Text(choice.theme.name == manager.iconOverrides[app.bundleIdentifier] ? "当前覆盖" : "点按以设置")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                        Spacer()
