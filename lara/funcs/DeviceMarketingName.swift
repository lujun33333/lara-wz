...
enum DeviceMarketingName {
    ...
    static var currentIdentifier: String {
        var value = utsname()
        uname(&value)
        let machine = withUnsafePointer(to: &value.machine) {
            $0.withMemoryRebound(to: CChar.self, capacity: 1) {
                String(cString: $0)
            }
        }

        if ["i386", "x86_64", "arm64"].contains(machine),
    ...
    static func name(for identifier: String) -> String {
        iPhoneNames[identifier] ?? identifier
    }

    static var currentName: String {
        name(for: currentIdentifier)
    }
}
